...
from app.services.relative_strength import (RelativeStrengthCalculator,
                                            RSRating, filter_by_rs_threshold,
                                            get_rs_emoji)
...
class TestRelativeStrengthCalculator:
    ...
    def test_percentile_ranking(self):
        ...
        assert (
            30.0 <= percentile <= 60.0
        ), f"Score of 50 should be near median, got {percentile}"

        percentile, rank = calc._calculate_percentile_rank(5.0, universe_scores)
        assert (
            percentile <= 10.0
        ), f"Score of 5 should be in bottom 10%, got {percentile}"
    ...
    def test_rs_rating_calculation(self):
        ...
        rs_rating = calc.calculate_rs_rating(
            symbol="STRONG", prices=strong_prices, universe_prices=universe_prices
        )
    ...
    def test_insufficient_data(self):
        ...
        rs_rating = calc.calculate_rs_rating(
            symbol="TEST", prices=prices, universe_prices=universe_prices
        )
...
class TestRSIntegration:
    ...
    def test_full_universe_ranking(self):
        ...
        rs_rating = calc.calculate_rs_rating(
            ...
            universe_prices={
                k: v for k, v in universe_prices.items() if k != "STOCK19"
            },
        )
        ...
        rs_rating_median = calc.calculate_rs_rating(
            ...
            universe_prices={
                k: v for k, v in universe_prices.items() if k != "STOCK10"
            },
        )

        assert rs_rating_median is not None
        assert (
            40.0 < rs_rating_median.percentile < 60.0
        ), "Median stock should be near 50th percentile"
...
class TestMinerviniFormula:
    ...
    def test_recent_performance_weighted_higher(self):
        ...
        assert (
            score_recent > score_consistent
        ), "Recent strong performance should score higher due to Q4 weighting"
