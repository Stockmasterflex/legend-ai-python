...
class Pattern:
    ...
    def to_dict(self) -> Dict:
        ...
        return {
            ...
            "expected_move": (
                round(self.expected_move, 2) if self.expected_move else None
            ),
            "win_probability": (
                round(self.win_probability, 2) if self.win_probability else None
            ),
...
class AdvancedPatternDetector:
    ...
    def detect_all_patterns(
        ...
        self.patterns_detected.extend(
            self._detect_double_triple_patterns(df, timeframe)
        )
        ...
        self.patterns_detected = [
            p for p in self.patterns_detected if p.confidence >= self.min_confidence
        ]
        ...
        logger.info(
            f"Detected {len(self.patterns_detected)} patterns above {self.min_confidence}% confidence"
        )
    ...
    def _detect_triangle_patterns(
        ...
        for window_size in [30, 45, 60]:
            ...
            # Descending Triangle: declining resistance + flat support
                ...
                peak_slope < -0.001 and abs(trough_slope) < 0.001 and abs(peak_r) > 0.8
            ):
            ...
            # Symmetrical Triangle: converging trendlines
                ...
                peak_slope < -0.001
                and trough_slope > 0.001
                and abs(peak_r) > 0.7
                and abs(trough_r) > 0.7
            ):
    ...
    def _detect_flag_pennant_patterns(
        ...
        # Pennant: converging trendlines (smaller triangle after strong move)
            ...
            if len(peaks_idx) >= 2 and len(troughs_idx) >= 2:
                ...
                peak_slope, _, _, _, _ = stats.linregress(
                    peaks_idx, cons_highs[peaks_idx]
                )
                trough_slope, _, _, _, _ = stats.linregress(
                    troughs_idx, cons_lows[troughs_idx]
                )
    ...
    def _detect_head_shoulders(self, df: pd.DataFrame, timeframe: str) -> List[Pattern]:
        ...
        for window_size in [40, 60, 80]:
            ...
            peaks_idx, peak_props = find_peaks(
                highs, distance=5, prominence=highs.std() * 0.5
            )
            ...
            if len(peaks_idx) >= 3:
                ...
                if (
                    peak_heights[1] > peak_heights[0] * 1.02
                    and peak_heights[1] > peak_heights[2] * 1.02
                    and abs(peak_heights[0] - peak_heights[2]) / peak_heights[0] < 0.05
                ):
                    ...
                    troughs_in_section_idx = argrelextrema(
                        trough_section["low"].values, np.less, order=3
                    )[0]

                    if len(troughs_in_section_idx) >= 2:
                        neckline = (
                            trough_section["low"].iloc[troughs_in_section_idx].mean()
                        )
                        ...
                        pattern = Pattern(
                            ...
                            key_points=[
                                (
                                    len(df) - window_size + int(last_3_peaks[0]),
                                    float(peak_heights[0]),
                                ),
                                (
                                    len(df) - window_size + int(last_3_peaks[1]),
                                    float(peak_heights[1]),
                                ),
                                (
                                    len(df) - window_size + int(last_3_peaks[2]),
                                    float(peak_heights[2]),
                                ),
                                (
                                    len(df) - window_size + int(last_3_peaks[1]),
                                    float(neckline),
                            ...
                            expected_move=-(
                                (peak_heights[1] - neckline) / current_price * 100
                            ),
            ...
            if len(troughs_idx) >= 3:
                ...
                if (
                    trough_depths[1] < trough_depths[0] * 0.98
                    and trough_depths[1] < trough_depths[2] * 0.98
                    and abs(trough_depths[0] - trough_depths[2]) / trough_depths[0]
                    < 0.05
                ):
                    ...
                    peaks_in_section_idx = argrelextrema(
                        peak_section["high"].values, np.greater, order=3
                    )[0]

                    if len(peaks_in_section_idx) >= 2:
                        neckline = (
                            peak_section["high"].iloc[peaks_in_section_idx].mean()
                        )
                        ...
                        pattern = Pattern(
                            ...
                            key_points=[
                                (
                                    len(df) - window_size + int(last_3_troughs[0]),
                                    float(trough_depths[0]),
                                ),
                                (
                                    len(df) - window_size + int(last_3_troughs[1]),
                                    float(trough_depths[1]),
                                ),
                                (
                                    len(df) - window_size + int(last_3_troughs[2]),
                                    float(trough_depths[2]),
                                ),
                                (
                                    len(df) - window_size + int(last_3_troughs[1]),
                                    float(neckline),
    ...
    def _detect_double_triple_patterns(
            ...
            if abs(peak_values[0] - peak_values[1]) / peak_values[0] < 0.02:
                ...
                if len(between_section) > 0:
                    ...
                    pattern = Pattern(
                        ...
                        key_points=[
                            (
                                len(df) - len(window) + int(last_2_peaks[0]),
                                float(peak_values[0]),
                            ),
                            (
                                len(df) - len(window) + int(last_2_peaks[1]),
                                float(peak_values[1]),
                            ),
                            (
                                len(df)
                                - len(window)
                                + int(last_2_peaks[0])
                                + int((last_2_peaks[1] - last_2_peaks[0]) / 2),
                                float(support),
                            ),
                        ],
                        target_price=target,
                        ...
                        expected_move=-(
                            (peak_values.mean() - support) / current_price * 100
                        ),
            ...
            if abs(trough_values[0] - trough_values[1]) / trough_values[0] < 0.02:
                ...
                if len(between_section) > 0:
                    ...
                    pattern = Pattern(
                        ...
                        key_points=[
                            (
                                len(df) - len(window) + int(last_2_troughs[0]),
                                float(trough_values[0]),
                            ),
                            (
                                len(df) - len(window) + int(last_2_troughs[1]),
                                float(trough_values[1]),
                            ),
                            (
                                len(df)
                                - len(window)
                                + int(last_2_troughs[0])
                                + int((last_2_troughs[1] - last_2_troughs[0]) / 2),
                                float(resistance),
                            ),
                        ],
                        target_price=target,
                        ...
                        expected_move=(
                            (resistance - trough_values.mean()) / current_price * 100
                        ),
        ...
        if len(peaks_idx) >= 3:
            ...
            if (max(peak_values) - min(peak_values)) / peak_values.mean() < 0.02:
                ...
                pattern = Pattern(
                    ...
                    key_points=[
                        (
                            len(df) - len(window) + int(last_3_peaks[i]),
                            float(peak_values[i]),
                        )
                        for i in range(3)
                    ],
        ...
        if len(troughs_idx) >= 3:
            ...
            if (max(trough_values) - min(trough_values)) / trough_values.mean() < 0.02:
                resistance = window.iloc[last_3_troughs[0] : last_3_troughs[2]][
                    "high"
                ].max()
                current_price = df["close"].iloc[-1]
                target = resistance + (resistance - trough_values.mean())
                ...
                pattern = Pattern(
                    ...
                    key_points=[
                        (
                            len(df) - len(window) + int(last_3_troughs[i]),
                            float(trough_values[i]),
                        )
                        for i in range(3)
                    ],
                    target_price=target,
    ...
    def _detect_wedge_patterns(self, df: pd.DataFrame, timeframe: str) -> List[Pattern]:
        ...
        for window_size in [30, 45, 60]:
            ...
            if (
                peak_slope > 0
                and trough_slope > 0
                and trough_slope > peak_slope  # Lower line rising faster (converging)
                and abs(peak_r) > 0.7
                and abs(trough_r) > 0.7
            ):
            ...
            # Falling Wedge: both lines falling, converging (bullish)
                ...
                peak_slope < 0
                and trough_slope < 0
                and peak_slope < trough_slope  # Upper line falling faster (converging)
                and abs(peak_r) > 0.7
                and abs(trough_r) > 0.7
            ):
    ...
    def _detect_gap_patterns(self, df: pd.DataFrame, timeframe: str) -> List[Pattern]:
        ...
        for i in range(1, min(20, len(df))):
            ...
            if curr_candle["low"] > prev_candle["high"] * 1.005:  # >0.5% gap
                gap_size = (
                    (curr_candle["low"] - prev_candle["high"])
                    / prev_candle["high"]
                    * 100
                )
            ...
            # Bearish Gap Down
                ...
                gap_size = (
                    (prev_candle["low"] - curr_candle["high"])
                    / prev_candle["low"]
                    * 100
                )
    ...
    def _detect_candlestick_patterns(
        ...
        if prev is not None and prev2 is not None:
            ...
            if (
                is_bearish(prev2)
                and body_size(prev) < body_size(prev2) * 0.5
                and is_bullish(last)
            ...
            if (
                ...
                and body_size(last) > body_size(prev2) * 0.7
            ):
                pattern = Pattern(
            ...
            for i in range(len(all_points) - 4):
                ...
                if (
                    ...
                    if XA > 0 and AB > 0 and BC > 0 and CD > 0:
                        ...
                        # Gartley Pattern
                        if (
                            0.55 < AB_XA_ratio < 0.68  # ~0.618
                            and 0.35 < BC_AB_ratio < 0.95  # 0.382-0.886
                            and 1.20 < CD_BC_ratio < 1.70
                        ):  # 1.272-1.618
                        ...
                        # Bat Pattern (different ratios)
                            ...
                            0.35 < AB_XA_ratio < 0.52  # ~0.382-0.500
                            and 0.35 < BC_AB_ratio < 0.95
                            and 1.60 < CD_BC_ratio < 2.70
                        ):  # 1.618-2.618
