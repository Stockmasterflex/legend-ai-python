...
class UniverseService:
    ...
    async def get_full_universe(self, dedupe: bool = True) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            logger.info(
                f"✅ Built universe: {len(universe)} tickers (SP500: {len(sp500)}, NASDAQ100: {len(nasdaq100)})"
            )
    ...
    async def scan_universe(
        ...
        try:
            ...
            logger.info(
                f"📊 Scanning {len(universe)} tickers from universe (SP500 + NASDAQ100)"
            )
            ...
            for i in range(
                0, len(universe), batch_size
            ):  # Scan ALL tickers in universe
                batch = universe[i : i + batch_size]
                logger.info(
                    f"📊 Scanning batch {i//batch_size + 1} ({len(batch)} tickers)..."
                )
                ...
                for item in batch:
                    ...
                    try:
                        ...
                        if cached_pattern:
                            ...
                            if pattern_result:
                                # Cache result
                                await self.cache.set_pattern(
                                    ticker, "1day", pattern_result.to_dict()
                                )

                        # Add to results if meets criteria
                        if pattern_result and pattern_result.score >= min_score:
                            if (
                                pattern_types is None
                                or pattern_result.pattern in pattern_types
                            ):
                                results.append(
                                    {
                                        "ticker": ticker,
                                ...
                                logger.info(
                                    f"✅ {ticker}: {pattern_result.pattern} ({pattern_result.score}/10)"
                                )
            ...
            logger.info(
                f"✅ Universe scan complete: {len(results)} setups found (min score: {min_score})"
            )
