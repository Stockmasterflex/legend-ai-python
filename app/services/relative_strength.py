...
class RelativeStrengthCalculator:
    ...
    def calculate_rs_rating(
        ...
        if len(prices) < self.MIN_DAYS_FOR_RS:
            logger.warning(f"{symbol}: Insufficient data ({len(prices)} days, need {self.MIN_DAYS_FOR_RS})")
            return None
        ...
        one_year_perf = ((prices[-1] - prices[-self.DAYS_PER_YEAR]) / prices[-self.DAYS_PER_YEAR]) * 100
    ...
    def _calculate_quarterly_performance(self, prices: np.ndarray) -> Optional[Tuple[float, float, float, float]]:
        """
        ...
        try:
            ...
            q4_perf = ((year_prices[-1] - year_prices[q4_start]) / year_prices[q4_start]) * 100
            
            # Q3: Days -126 to -64
            q3_perf = ((year_prices[q4_start] - year_prices[q3_start]) / year_prices[q3_start]) * 100
            
            # Q2: Days -189 to -127
            q2_perf = ((year_prices[q3_start] - year_prices[q2_start]) / year_prices[q2_start]) * 100
            
            # Q1 (oldest): Days -252 to -190
            q1_perf = ((year_prices[q2_start] - year_prices[q1_start]) / year_prices[q1_start]) * 100
    ...
    def _calculate_universe_scores(self, universe_prices: Dict[str, np.ndarray]) -> Dict[str, float]:
        """
    ...
    def calculate_rs_simple(
        ...
        stock_return = ((stock_prices[-1] - stock_prices[-61]) / stock_prices[-61]) * 100
        bench_return = ((bench_prices[-1] - bench_prices[-61]) / bench_prices[-61]) * 100
