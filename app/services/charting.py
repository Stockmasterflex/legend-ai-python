...
class ChartingService:
    ...
    def __init__(self):
        ...
        logger.info(
            f"🎨 ChartingService initialized - API key present: {bool(self.api_key)}"
        )
        if not self.api_key:
            logger.warning(
                "⚠️ Chart-IMG API key not found in any environment variable (CHART_IMG_API_KEY, CHARTIMG_API_KEY, etc.)"
            )
        else:
            logger.info(
                f"🎨 Chart-IMG API key loaded successfully (length: {len(self.api_key)})"
            )
    ...
    async def _check_rate_limit(self):
        ...
        try:
            ...
            if usage >= self.daily_limit:
                logger.warning(
                    "⚠️ Chart-IMG daily quota exhausted. Using fallback charts."
                )
            ...
            logger.error(
                f"⚠️ Rate limit check failed: {e}. Allowing request to proceed."
            )
    ...
    def _get_fallback_url(self, ticker: str, timeframe: str = "1day") -> str:
        ...
        """
        logger.warning(
            f"📊 Returning None for {ticker} ({timeframe}) - Chart-IMG unavailable"
        )
        return None
    ...
    def _build_long_position_drawing(
        ...
        drawing = {
            ...
            "override": {"fillBackground": True, "showPrice": True, "showStats": True},
        }
        return drawing
    ...
    async def generate_chart(
        ...
        """
        logger.info(
            f"🎨 Starting chart generation for {ticker} on {timeframe} with preset {preset}"
        )
        ...
        if not self.api_key or self.api_key.lower().startswith("dev"):
            ...
            raise ValueError(
                "Chart-IMG API key not configured. Please set the CHART_IMG_API_KEY environment variable."
            )
        ...
        try:
            ...
            logger.info(
                f"📊 Rate limit check for {ticker}: {'PASS' if rate_limit_ok else 'FAIL'}"
            )
            ...
            logger.info(
                f"📡 Making Chart-IMG API request for {ticker} with API key present: {bool(self.api_key)}"
            )
            max_retries = 4
            ...
            for attempt in range(max_retries):
                try:
                    ...
                    logger.info(
                        f"📡 Chart-IMG response status: {response.status_code} for {ticker} (attempt {attempt + 1}/{max_retries}, {duration:.2f}s)"
                    )
                    ...
                    if response.status_code == 200:
                        ...
                        if attempt < max_retries - 1:
                            logger.warning(
                                f"⏳ Server error, retrying in {retry_delay}s..."
                            )
                            await asyncio.sleep(retry_delay)
                            retry_delay *= 2
                            continue
                    ...
                    if attempt < max_retries - 1:
                        logger.warning(
                            f"⏳ Network error, retrying in {retry_delay}s: {e}"
                        )
                        await asyncio.sleep(retry_delay)
                        retry_delay *= 2
                        continue
            ...
            if response.status_code == 200:
                ...
                if data.get("success") and data.get("data"):
                    ...
                    chart_url = (
                        data.get("url") or data.get("imageUrl") or data.get("image_url")
                    )
                    expires_at = data.get("expires_at", "N/A")

                if chart_url:
                    logger.info(
                        f"✅ Chart generated for {ticker} ({timeframe}): {chart_url[:60]}... (expires: {expires_at})"
                    )
                    ...
                    if self.redis:
                        try:
                            cache_key = (
                                f"chartimg:{ticker}:{timeframe}:{entry}:{stop}:{target}"
                            )
                    ...
                    logger.warning(
                        f"⚠️ Chart-IMG response missing URL for {ticker}: {data}"
                    )
                    return self._get_fallback_url(ticker, timeframe)
            else:
                logger.warning(
    ...
    async def generate_thumbnail(
        self, ticker: str, timeframe: str = "1day", preset: str = "breakout"
    ) -> Optional[str]:
        """Generate lightweight thumbnails for watchlist cards."""
        return await self.generate_chart(
            ticker=ticker, timeframe=timeframe, width=400, height=225, preset=preset
        )
    ...
    async def get_chart_batch(
        self, tickers: List[str], timeframe: str = "1day"
