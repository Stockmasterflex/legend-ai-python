    ...
    async def run(
        ...
        for i, window in enumerate(windows):
            try:
                ...
                degradation = 0.0
                if is_metric != 0:
                    degradation = ((is_metric - oos_results.get(self.config.optimization_metric, 0)) / abs(is_metric)) * 100
                ...
                result = WindowResult(
                    ...
                    out_sample_metric=oos_results.get(self.config.optimization_metric, 0),
        ...
        avg_sharpe = sum(combined_oos_sharpes) / len(combined_oos_sharpes) if combined_oos_sharpes else 0
        
        # Calculate robustness score (% of windows with positive OOS)
        positive_windows = sum(1 for r in self.window_results if r.out_sample_return > 0)
        robustness_score = (positive_windows / len(self.window_results) * 100) if self.window_results else 0
        ...
        return {
            ...
            "avg_degradation": sum(r.degradation_pct for r in self.window_results) / len(self.window_results) if self.window_results else 0,
            "window_results": [
                {
