...
class MonteCarloConfig:
    ...
    simulation_types: List[str] = field(
        default_factory=lambda: ["random_entry", "position_size"]
    )
...
class MonteCarloEngine:
    ...
    async def run(
        ...
        logger.info(
            f"Starting Monte Carlo simulation with {self.config.n_simulations} iterations"
        )
            ...
            try:
                ...
                for sim_type in self.config.simulation_types:
                    if sim_type == "random_entry":
                        ...
                        sim_trades, mod = self._apply_position_size_variation(
                            sim_trades
                        )
                        modifications.update(mod)
    ...
    def _apply_random_entry(self, trades: List[Dict]) -> tuple:
        ...
        for trade in trades:
            delay = random.randint(
                self.config.entry_delay_min_days, self.config.entry_delay_max_days
            )
            delays.append(delay)
        ...
        return modified_trades, {
            "avg_delay_days": statistics.mean(delays) if delays else 0
        }
    ...
    def _apply_position_size_variation(self, trades: List[Dict]) -> tuple:
        ...
        return modified_trades, {
            "avg_size_factor": statistics.mean(size_factors) if size_factors else 1
        }
    ...
    def _apply_bootstrap(self, trades: List[Dict]) -> tuple:
        ...
        return resampled, {
            "bootstrap": True,
            "unique_trades": len(set(id(t) for t in resampled)),
        }
    ...
    def _compile_results(
        ...
        cvar_95 = (
            statistics.mean(returns_sorted[: cvar_95_idx + 1])
            if cvar_95_idx >= 0
