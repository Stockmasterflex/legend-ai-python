        ...
        try:
            ...
            if status == "Watching" and entry:
                ...
                if current_price > entry and volume_ratio > 1.5:
                    ...
                    alert_type = "breakout"
                    logger.info(f"🚀 {ticker} breaking out at ${current_price:.2f} (entry: ${entry:.2f}, volume: {volume_ratio:.1f}x)")
                ...
                if stop and current_price < stop:
                    ...
                    alert_type = "stop_hit"
                    logger.info(f"🛑 {ticker} stop hit at ${current_price:.2f} (stop: ${stop:.2f})")
                
                # Target hit
                elif target and current_price > target:
                    new_status = "Triggered"
                    alert_type = "target_hit"
                    logger.info(f"🎯 {ticker} target hit at ${current_price:.2f} (target: ${target:.2f})")
            
            # Update status if changed
            if new_status != status:
                self.db.update_watchlist_symbol(
                    ticker,
                    status=new_status
                )
            ...
            for item in items:
                ...
                if alert:
                    ...
                    try:
                        ...
                        logger.warning(f"Failed to log alert for {alert['ticker']}: {e}")
                
                # Small delay to avoid rate limits
                await asyncio.sleep(0.1)
            
            if alerts:
                logger.info(f"✅ Watchlist check complete: {len(alerts)} alerts triggered")
            else:
