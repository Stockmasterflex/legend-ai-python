                ...
                sources.append(
                    {
                        ...
                        "percent_of_total": round(
                            (stats["used"] / max(total_calls, 1)) * 100, 1
                        ),
                        "cost_per_call": _get_cost_per_call(source_name),
                        "daily_cost": round(
                            stats["used"] * _get_cost_per_call(source_name), 4
                        ),
                    }
                )
...
def _calculate_cost_projections(usage_stats: Dict, cache_stats: Dict) -> Dict[str, Any]:
    ...
    unoptimized_costs = {
        "twelvedata": usage_stats.get("twelvedata", {}).get("used", 0)
        * 0.01,  # Hypothetical paid tier
...
def _generate_recommendations(
    usage_stats: Dict, cache_stats: Dict
) -> List[Dict[str, Any]]:
    ...
    total_calls = sum(
        api.get("used", 0) for api in usage_stats.values() if isinstance(api, dict)
    )
    if total_calls > 100:
        recommendations.append(
            {
