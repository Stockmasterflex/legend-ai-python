...
def calculate_position_size(
    entry: float, stop: float, account_size: float, risk_percent: float
...
def calculate_partial_exits(
    entry: float, stop: float, target: float, position_size: int
) -> List[PartialExit]:
    """
    ...
    return [
        PartialExit(
            shares=shares_1r, price=round(price_1r, 2), r_multiple=1.0, percentage=50.0
        ),
        PartialExit(
            shares=shares_2r, price=round(price_2r, 2), r_multiple=2.0, percentage=30.0
        ),
        PartialExit(
            shares=shares_3r, price=round(price_3r, 2), r_multiple=3.0, percentage=20.0
        ),
    ]
    ...
    **Returns:**
    ...
    try:
        ...
        if request.entry <= request.stop:
            raise HTTPException(
                status_code=400, detail="Entry must be above stop for long positions"
            )

        if request.target <= request.entry:
            raise HTTPException(
                status_code=400, detail="Target must be above entry for long positions"
            )

        # Calculate position size
        position_size, dollar_amount, risk_amount = calculate_position_size(
            request.entry, request.stop, request.account_size, request.risk_percent
        )
        ...
        partial_exits = calculate_partial_exits(
            request.entry, request.stop, request.target, position_size
        )
...
async def quick_position_size(
    entry: float, stop: float, account_size: float, risk_percent: float = 1.0
):
