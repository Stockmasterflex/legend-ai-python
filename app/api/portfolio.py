...
Endpoints for portfolio and position tracking
"""
...
class RemovePositionRequest(BaseModel):
    ...
    quantity: Optional[float] = Field(None, description="Quantity to sell (None = full position)")
    exit_price: Optional[float] = Field(None, description="Exit price (None = current market price)")
...
@router.get("/list", summary="Get user portfolios")
