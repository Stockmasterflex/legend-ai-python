...
async def log_trade(trade: TradeCreate):
    ...
    try:
        ...
        logger.info(
            f"Trade logged: {trade.ticker} @ ${trade.entry_price} x {trade.shares} shares"
        )
...
async def get_trades(
    status: Optional[str] = None, ticker: Optional[str] = None, limit: int = 100
):
...
async def update_trade(trade_id: int, update: TradeUpdate):
    ...
    try:
        ...
        with db.engine.connect() as conn:
            result = conn.execute(
                text(
                    "SELECT entry_price, stop_price, shares FROM trades WHERE id = :id"
                ),
                {"id": trade_id},
            )
            row = result.fetchone()
...
async def get_stats():
    ...
    try:
        ...
        expectancy = (
            (win_rate / 100 * avg_win) - (loss_rate / 100 * abs(avg_loss))
            if closed_trades > 0
            else 0.0
        )
...
async def export_trades():
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to export trades: {str(e)}"
        )
