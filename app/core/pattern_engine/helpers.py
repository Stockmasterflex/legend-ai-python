...
class PatternHelpers:
    ...
    def find_bottom_spike_length(
        self, highs: np.ndarray, lows: np.ndarray, pattern_start: int, pattern_end: int
    ) -> Tuple[float, float]:
        """
    ...
    def wide_range_average(
        self, highs: np.ndarray, lows: np.ndarray, end_idx: int, lookback: int = 22
    ) -> float:
        """
    ...
    def hl_regression(
        ...
        if use_closes:
            ...
            series = (
                highs[start_idx : end_idx + 1] + lows[start_idx : end_idx + 1]
            ) / 2.0
        ...
        if end_idx + 1 < len(highs) and end_idx - 1 >= 0:
            if (
                direction == -1
                and highs[end_idx] < highs[end_idx + 1]
                and highs[end_idx - 1] < highs[end_idx]
            ):
                direction = 1
            elif (
                direction == 1
