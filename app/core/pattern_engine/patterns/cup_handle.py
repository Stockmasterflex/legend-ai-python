...
def find_cup(
    data: PatternData, helpers: PatternHelpers, strict: bool = False
) -> List[Dict[str, Any]]:
    """
    ...
    tops = helpers.find_all_tops(
        data.highs, data.chart_start_index, data.chart_end_index, trade_days
    )
    ...
    for i in range(len(tops) - 1, 0, -1):
        ...
        for j in range(i - 1, -1, -1):
            ...
            for k in range(
                right_rim_idx + 1, min(len(data.closes), data.chart_end_index + 1)
            ):
                ...
                patterns.append(
                    {
                        ...
                        "mid_idx": right_rim_idx,
                        "end_idx": min(
                            len(data.closes) - 1, right_rim_idx + cup_width // 4
                        ),
                        "left_rim": left_rim_high,
                        "right_rim": right_rim_high,
                        "bottom": bottom_low,
