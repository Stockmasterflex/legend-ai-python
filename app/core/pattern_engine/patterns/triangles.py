...
def find_ascending_triangle(
    ...
    for resist in tl_resistance:
        for support in tl_support:
            ...
            for k in range(
                resist["end_idx"] + 1, min(len(data.closes), resist["end_idx"] + 30)
            ):
...
def find_descending_triangle(
    ...
    for support in tl_support:
        for resist in tl_resistance:
            ...
            for k in range(
                support["end_idx"] + 1, min(len(data.closes), support["end_idx"] + 30)
            ):
...
def find_sym_triangle(
    ...
    tops = helpers.find_all_tops(
        data.highs, data.chart_start_index, data.chart_end_index, trade_days=3
    )
    bottoms = helpers.find_all_bottoms(
        data.lows, data.chart_start_index, data.chart_end_index, trade_days=3
    )
    ...
    for resist in resistance_lines:
        for support in support_lines:
            ...
            if breakout_direction == "up":
                ...
                target = (
                    _get_tl_price(resist, overlap_end)
                    + _get_tl_price(support, overlap_end)
                ) / 2
...
def _find_triangle_trendlines(
    data: PatternData, helpers: PatternHelpers, triangle_type: str
    ...
    if triangle_type == "ascending":
        ...
        resistance_lines = _find_horizontal_levels(
            data, helpers, tops, data.highs, is_high=True
        )
        # Rising support: find ascending line through bottoms
        support_lines = _find_ascending_trendlines(
            data, helpers, bottoms, min_touches=2
        )

    elif triangle_type == "descending":
        # Flat support: find bottoms at similar depths
        support_lines = _find_horizontal_levels(
            data, helpers, bottoms, data.lows, is_high=False
        )
        # Falling resistance: find descending line through tops
        resistance_lines = _find_descending_trendlines(
            data, helpers, tops, min_touches=2
        )
...
def _find_ascending_trendlines(
    data: PatternData, helpers: PatternHelpers, pivots: np.ndarray, min_touches: int = 2
) -> List[Dict]:
    ...
    for i in range(len(pivots) - 1):
        for j in range(i + 1, len(pivots)):
            ...
            for k in range(i + 1, j):
                ...
                if helpers.check_nearness(
                    expected, data.lows[pivots[k]], price_vary=0.25
                ):
    ...
    for i in range(len(pivots) - 1):
        for j in range(i + 1, len(pivots)):
            ...
            for k in range(i + 1, j):
                expected = price1 + slope * (pivots[k] - pivots[i])
                if helpers.check_nearness(
                    expected, data.highs[pivots[k]], price_vary=0.25
                ):
                    touches.append(pivots[k])
...
def _get_tl_price(trendline: Dict, idx: int) -> float:
    ...
    if "level" in trendline:
        ...
        # Sloped line
        return trendline["start_price"] + trendline["slope"] * (
            idx - trendline["start_idx"]
        )


def _calculate_triangle_confidence(
    touches1: int, touches2: int, width: int, confirmed: bool
) -> float:
