...
def find_wedges(
    ...
    rising_wedges = _find_wedge_type(
        high, low, close, helpers, wedge_type="rising", strict=strict
    )
    patterns.extend(rising_wedges)

    # Find falling wedges (Flag=96)
    falling_wedges = _find_wedge_type(
        high, low, close, helpers, wedge_type="falling", strict=strict
    )
    ...
    for lower_tl in lower_trendlines:
        ...
        for upper_tl in upper_trendlines:
            ...
            pattern_height = float(
                np.max(high[wedge_start : wedge_end + 1])
                - np.min(low[wedge_start : wedge_end + 1])
            )
...
def _find_trendlines(
    ...
    for i in range(len(pivots) - 1):
        ...
        for j in range(i + 1, len(pivots)):
            ...
            slope = (
                price_change / (primary[start_idx] * time_change)
                if primary[start_idx] != 0
                else 0
            )
            ...
            for k in range(start_idx, end_idx + 1):
                ...
                expected = primary[start_idx] + (k - start_idx) * (
                    primary[end_idx] - primary[start_idx]
                ) / (end_idx - start_idx)
