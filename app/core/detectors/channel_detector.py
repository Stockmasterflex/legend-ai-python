...
class ChannelDetector(Detector):
    ...
    def _detect_channel(
        ...
        if channel_type == 'up':
            ...
            avg_price = np.mean([h['price'] for h in highs] + [l['price'] for l in lows])
        ...
        window_start = ohlcv.index[first_idx].isoformat() if hasattr(ohlcv.index[first_idx], 'isoformat') else str(ohlcv.index[first_idx])
        window_end = ohlcv.index[last_idx].isoformat() if hasattr(ohlcv.index[last_idx], 'isoformat') else str(ohlcv.index[last_idx])
