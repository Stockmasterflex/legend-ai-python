...
def zigzag_peaks_troughs(prices: pd.Series, pct_threshold: float = 0.1):
    ...
    pivots=[]; last_price=prices.iloc[0]; direction=0
    for i in range(1,len(prices)):
        chg=(prices.iloc[i]-last_price)/last_price
        if direction>=0 and chg<=-pct_threshold:
            pivots.append((i-1,"peak")); direction=-1; last_price=prices.iloc[i-1]
        elif direction<=0 and chg>=pct_threshold:
            pivots.append((i-1,"trough")); direction=1; last_price=prices.iloc[i-1]
        else:
            if direction>=0 and prices.iloc[i]>last_price: last_price=prices.iloc[i]
            elif direction<=0 and prices.iloc[i]<last_price: last_price=prices.iloc[i]
    return pivots
...
class PatternDetectors:
    ...
    # -------------------- CUP WITH HANDLE --------------------
    @staticmethod
    def cup_with_handle(df: pd.DataFrame, cfg: DetectionConfig=DetectionConfig())->List[PatternSignal]:
        try:
            df = ensure_ohlcv(df)
            ...
            for k in range(len(piv)-2):
                i1,t1=piv[k]; i2,t2=piv[k+1]; i3,t3=piv[k+2]
                if not (t1=="peak" and t2=="trough" and t3=="peak"): continue
                start,end=df.index[i1],df.index[i3]
                weeks=(end-start).days/7
                if not(cfg.cup_min_weeks<=weeks<=cfg.cup_max_weeks): continue
                left,right,bottom=close.iloc[i1],close.iloc[i3],close.iloc[i2]
                depth=(left-bottom)/left if left>0 else np.nan
                if not(cfg.cup_depth_min<=depth<=cfg.cup_depth_max): continue
                if abs((right-left)/left)>0.05: continue

                # handle
                hstart=i3+1
                if hstart>=len(df): continue
                ...
                if hdepth>cfg.handle_max_depth: continue
                mid=bottom+0.5*(left-bottom)
                if handle_low<mid: continue

                avg50 = vol.rolling(cfg.vol_avg_window).mean()
                hvol = vol.iloc[hstart:handle_low_i+1]
                vol_dry = (hvol.mean() < avg50.iloc[handle_low_i]*cfg.vol_dryup_percentile)

                post=df.iloc[handle_low_i+1:handle_low_i+15]
                breakout=None
                for j in range(len(post)):
                    if post["close"].iloc[j]>right and post["volume"].iloc[j]>=cfg.breakout_vol_mult*avg50.iloc[post.index[j]]:
                        breakout=post.index[j]; break
                if breakout is None: continue

                score=min(1.0, 0.6 + 0.15*vol_dry + 0.1*(depth<0.25) + 0.15*(hdepth<0.12))
                sigs.append(PatternSignal(
                    "Cup with Handle", float(right*1.0001), breakout, score,
                    Window(start,end),
                    {"depth":float(depth), "handle_depth":float(hdepth), "weeks":float(weeks), "vol_dryup":bool(vol_dry)}
                ))
            return sigs
    ...
    # -------------------- FLAT BASE --------------------------
    @staticmethod
    def flat_base(df: pd.DataFrame, cfg: DetectionConfig=DetectionConfig())->List[PatternSignal]:
        try:
            df = ensure_ohlcv(df)
            w=to_weekly(df)
            sigs=[]
            ...
            for s in range(0, len(w)-cfg.flat_min_weeks-2):
                for win in range(cfg.flat_min_weeks, 9):
                    ...
                    if depth>cfg.flat_max_depth: continue
                    first_high=w["high"].iloc[s]
                    if (peak-first_high)/first_high>0.01: continue
                    tight = seg["close"].std()/max(1e-9, seg["close"].mean()) < 0.015
                    if not tight: continue
                    if s<12: continue
                    pre=w.iloc[s-12:s]
                    pre_ret=pre["close"].iloc[-1]/pre["close"].iloc[0]-1 if pre["close"].iloc[0]>0 else 0
                    if pre_ret<0.30: continue
                    pivot=peak
                    post=w.iloc[e+1:min(e+4, len(w))]
                    wk=None
                    for i in range(len(post)):
                        if post["close"].iloc[i]>pivot and post["volume"].iloc[i]>=cfg.breakout_vol_mult*avg_vol.iloc[post.index[i]]:
                            wk=post.index[i]; break
                    if wk is None: continue
                    daily_break=df[df.index>=wk]
                    br=daily_break[daily_break["close"]>pivot].index.min()
                    if pd.isna(br): continue
                    score=min(1.0, 0.6 + 0.2*(depth<0.12) + 0.2*tight)
                    sigs.append(PatternSignal(
                        "Flat Base", float(pivot*1.0001), br, score,
                        Window(w.index[s], br),
                        {"weeks":float(win), "depth":float(depth), "pre_uptrend":float(pre_ret)}
                    ))
            return sigs
        except Exception as e:
    ...
    # -------------------- 21 EMA PULLBACK --------------------
    @staticmethod
    def pullback_21ema(df: pd.DataFrame, cfg: DetectionConfig=DetectionConfig())->List[PatternSignal]:
        try:
            df = ensure_ohlcv(df)
            close,vol=df["close"],df["volume"]
            ema21=ema(close,21); sma50=sma(close,50); sma200=sma(close,200)
            avg_vol=vol.rolling(cfg.vol_avg_window).mean()
            trend=(close>sma50)&(close>sma200)&(sma50>sma50.shift(1))
            sigs=[]
            for i in range(21,len(df)-1):
                if not trend.iloc[i-1]: continue
                if i<cfg.ema21_lookback_above: continue
                if (df["low"].iloc[i]<=ema21.iloc[i]) and (close.iloc[i]>=ema21.iloc[i]):
                    under=max(0,(ema21.iloc[i]-df["low"].iloc[i])/max(1e-9, ema21.iloc[i]))
                    if under<=cfg.ema21_max_undercut:
                        pull_vol = vol.iloc[i] <= avg_vol.iloc[i]
                        score=min(1.0, 0.6 + 0.2*pull_vol + 0.2*(close.iloc[i]>close.iloc[i-1]))
                        sigs.append(PatternSignal(
                            "21EMA Pullback", float(max(close.iloc[i], ema21.iloc[i])),
                            df.index[i], score, Window(df.index[max(0,i-10)], df.index[i]),
                            {"undercut":float(under), "pull_vol_ok":bool(pull_vol)}
                        ))
            return sigs
        except Exception as e:
    ...
    @staticmethod
    def vcp(df: pd.DataFrame, cfg: DetectionConfig=DetectionConfig())->List[PatternSignal]:
        """
        VCP detection:
        ...
        try:
            ...
            recent = df.iloc[-cfg.vcp_recent_days:].copy() if len(df) >= cfg.vcp_recent_days else df.copy()
            ...
            contractions = []
            for hi in highs_idx:
                hi_ts = recent.index[hi]; hi_price = recent["high"].iloc[hi]
                ...
                lo_ts = recent.index[lo]; lo_price = recent["low"].iloc[lo]
                pct_drop = (hi_price - lo_price) / max(1e-9, hi_price)
                avg_vol = recent["volume"].iloc[hi:lo+1].mean() if lo>hi else np.nan
                contractions.append({
                    "start_idx": int(hi), "end_idx": int(lo),
                    "start": hi_ts, "end": lo_ts,
                    "high": float(hi_price), "low": float(lo_price),
                    "percent_drop": float(pct_drop), "avg_volume": float(avg_vol),
            ...
            vols = np.array([c["avg_volume"] for c in contractions if np.isfinite(c["avg_volume"])])
            vol_down_ok = True
            if len(vols) >= 3:
            ...
            if len(contractions) >= 2:
                ...
                if np.isfinite(prev_vol) and prev_vol > 0 and np.isfinite(last_vol):
                    vol_dryup = ((prev_vol - last_vol) / prev_vol) >= cfg.vcp_volume_dry_up_drop
            ...
            vol_break   = (df["volume"].iloc[-1] >= cfg.vcp_breakout_vol_mult * max(1e-9, avg50.iloc[-1]))
            ...
            n = len(contractions)
            if 3 <= n <= 4: score += 0.25
            elif 2 <= n <= 5: score += 0.10
            # final tightness
            if final_pct <= 0.05: score += 0.20
            elif final_pct <= 0.08: score += 0.10
            # volume profile
            if vol_down_ok: score += 0.10
            if vol_dryup:   score += 0.15
            # breakout bonus (not required to issue a setup)
            if broke_out:   score += 0.05
            ...
            return [PatternSignal(
                "VCP", float(pivot), today, score, window,
                {
    ...
    @staticmethod
    def double_bottom(df: pd.DataFrame, cfg: DetectionConfig=DetectionConfig()) -> List[PatternSignal]:
        return []
    @staticmethod
    def ascending_base(df: pd.DataFrame, cfg: DetectionConfig=DetectionConfig()) -> List[PatternSignal]:
        return []
    @staticmethod
    def base_on_base(df: pd.DataFrame, cfg: DetectionConfig=DetectionConfig()) -> List[PatternSignal]:
        return []
    @staticmethod
    def ipo_base(df: pd.DataFrame, cfg: DetectionConfig=DetectionConfig()) -> List[PatternSignal]:
        return []
    @staticmethod
    def high_tight_flag(df: pd.DataFrame, cfg: DetectionConfig=DetectionConfig()) -> List[PatternSignal]:
        return []
    @staticmethod
    def three_weeks_tight(df: pd.DataFrame, cfg: DetectionConfig=DetectionConfig()) -> List[PatternSignal]:
        return []
    @staticmethod
    def tight_areas(df: pd.DataFrame, cfg: DetectionConfig=DetectionConfig()) -> List[PatternSignal]:
        return []
...
def _trend_template_gate(df: pd.DataFrame) -> Tuple[bool, float, Dict[str, Any]]:
    ...
    try:
        ...
        return (passed >= 6), float(score), {
            "passed": passed, "total": len(crit),
            "price": float(price), "ma50": float(m50), "ma150": float(m150), "ma200": float(m200)
        }
    except Exception as e:
...
def scan_all(df: pd.DataFrame, cfg: DetectionConfig=DetectionConfig()) -> List[PatternSignal]:
    """
